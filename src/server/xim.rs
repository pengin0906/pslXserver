...
pub struct XimAtoms {
    ...
    pub server_atom: u32,      // @server=xserver
...
impl XimServer {
    pub fn new(atoms: &AtomTable, root_window: Xid) -> Self {
        let xim_atoms = XimAtoms {
            ...
            server_atom: atoms.intern_or_create("@server=xserver"),
            locales: atoms.intern_or_create("LOCALES"),
            transport: atoms.intern_or_create("TRANSPORT"),
        };

        info!("XIM server initialized: XIM_SERVERS={}, _XIM_XCONNECT={}, _XIM_PROTOCOL={}, @server=xserver={}",
    ...
    pub fn handle_selection_request(
        ...
        debug!("XIM: SelectionRequest for @server=xserver, target={}, requestor=0x{:08x}", target, requestor);
