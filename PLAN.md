...
3. `src/server/xim.rs` 582行目: `XIM_SET_EVENT_MASK` の `forward_event_mask` を
   ...
   - これにより Chrome は KeyPress を XIM サーバー（Xerver）に転送してくる
   - Xerver は非IME中のキーはそのまま `XIM_FORWARD_EVENT` で Chrome に返す
...
フロー:
```
クライアント → XIM_FORWARD_EVENT(KeyPress) → Xerver
  IME中でない場合:
    Xerver → XIM_FORWARD_EVENT(KeyPress, flag=0) → クライアント
  IME中の場合:
...
`forward_event_mask` を正しく設定すれば、Xerver が全キーイベントのゲートキーパーになる。
...
## テスト手順
1. Xerver を `--tcp` 付きで起動（XIM有効化ビルド）
